...
# #         self.embedding.requres_grad = False # do not train label embedding
#         # for y_hat
...
# #         labels_embedding = self.embedding(y_hat_t)  # N, Embedding
        y_hat_t_embedding = self.embedding(y_hat_t_label)
        rnn_input = torch.cat(
...
#             y_hat_t = self.fc2(torch.cat((query.squeeze(1), attention_context), dim=1)) # N, vocab_size
            ...
            y_hat_smoothed = self.gumble_softmax(y_hat_t, temperature=1, eps=1e-10) # L, N, Vocabsize
...
#                     labels_smoothed[time_index, :, :])  # N, emb
                y_hat_t_embedding = self.embedding(labels_padded[time_index, :]) # N, Embedding
                rnn_input = torch.cat(
                    (y_hat_t_embedding, attention_context), dim=1)
    ...
    def beam_search_decode_inference(self, utterance_key, utterance_value, utterance_seq_len, SOS_token, EOS_token, max_label_len, beam_size, num_candidates):
        ...
        for time_index in range(max_label_len):
            ...
            for beam_node in beam_nodes:
                ...
                # get top n candidate
                logits = F.log_softmax(y_hat_t_gumbel, dim=1)
                ...
                for candidate_index in range(beam_size):
                    new_beam_node = {
                        ...
                        'y_hats': beam_node['y_hats']+[y_hat_t_gumbel] if len(beam_node['y_hats'])!=0 else [torch.zeros_like(y_hat_t), y_hat_t_gumbel],
