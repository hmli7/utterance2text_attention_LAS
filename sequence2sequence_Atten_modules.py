...
class Encoder_RNN(nn.Module):
    def __init__(self, embed_size, hidden_size, n_plstm, mlp_hidden_size, mlp_output_size):
        ...
        self.rnns.append(nn.LSTM(input_size=embed_size,
                                 hidden_size=hidden_size, num_layers=nlayers, bidirectional=True))
...
class Decoder_RNN(nn.Module):
    def __init__(self, vocab_size, embed_size, key_value_size, hidden_size, n_layers, mlp_hidden_size):
        ...
        self.embedding = nn.Embedding(
            self.vocab_size, self.embed_size, padding_idx=-1)
    ...
    def forward(self, key, value, labels, final_seq_lens, seq_order, reversed_seq_order, SOS_token, TEACHER_FORCING_Ratio=0, TEST=False):
        ...
        labels_padded = pad_sequence(labels, padding_value=-1)
        # embedding
        ...
        for i in range(self.n_layers-1):
            ...
            hidden_size.append(self.init_states(key))
            # [n_layers, N, Hidden_size]
            memory_states.append(self.init_states(key))

        max_label_len = labels_embedding.size(1)
        ...
        key_mask = key_mask.unsqueeze(2).repeat(
            (1, 1, max_label_len))  # expend to N, max_key_len, max_label_len

        # ------ LAS ------
        # initialze for the first time step input
        y_hat_t_label = torch.Tensor([SOS_token]*batch_size)  # N
        y_hat = []
        ...
        for time_index in range(max_label_len):
            ...
            y_hat.append(y_hat_t.unsqueeze(1)) # N, 1, vocab_size

            y_hat_t_label = torch.argmax(y_hat_t, dim=1) # N
        # concat predictions
        y_hat = torch.stack(y_hat, dim=1) # N, label_L, vocab_size
        return y_hat, labels_padded, labels_lens
...
class Sequence2Sequence(nn.Module):
    ...
    def forward(self, seq_batch, TEACHER_FORCING_Ratio=0, TEST=False):
        keys, values, labels, final_seq_lens, sequence_order, reverse_sequence_order = self.encoder(
            seq_batch)
        y_hat, labels_padded, labels_lens = self.decoder(keys, values, labels, final_seq_lens, sequence_order,
                            reverse_sequence_order, char_language_model.SOS_token, TEACHER_FORCING_Ratio=TEACHER_FORCING_Ratio, TEST=TEST)
        if TEST:
            pass
